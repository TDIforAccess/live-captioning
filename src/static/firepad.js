...
(function (name, definition, context) {
	...
	firepad.TextOp = (function () {
		...
		function TextOp(type) {
			...
			if (type === 'insert') {
				...
				this.color = arguments[3] ? arguments[3].color : undefined;
	...
	firepad.TextOperation = (function () {
		'use strict';
		var TextOp = firepad.TextOp; // operation on text
		var utils = firepad.utils;
    ...
    // meta holds operation metas
		TextOperation.prototype.retain = function (n, attributes, meta) {
			...
			if (prevOp && prevOp.isRetain() && prevOp.attributesEqual(attributes)) {
				...
				// Create a new op.
				this.ops.push(new TextOp('retain', n, attributes, meta));
			}
			return this;
		};

		// Insert a string at the current position.
		TextOperation.prototype.insert = function (str, attributes, meta) {
      if (typeof str !== 'string') {
			...
			if (prevOp && prevOp.isInsert() && prevOp.attributesEqual(attributes)) {
				...
				if (prevPrevOp && prevPrevOp.isInsert() && prevPrevOp.attributesEqual(attributes)) {
					...
					this.ops[this.ops.length - 1] = new TextOp('insert', str, attributes, meta);
					this.ops.push(prevOp);
				}
			} else {
				this.ops.push(new TextOp('insert', str, attributes, meta));
			}
			return this;
		};
		...
		TextOperation.fromJSON = function (data) {
          ...
          attributes = {}
					if (op > 0) {
						o.retain(op, attributes, {color: data.c});
					} else {
						o['delete'](-op);
					}
				} else {
					utils.assert(typeof op === 'string');
					o.insert(op, attributes, {color: data.c});
				}
			}
			return o;
		};
...
// so all operations are O(n), where n is the number of spans in the list.
	...
	firepad.FirebaseAdapter = (function (global) {
		...
		function FirebaseAdapter(realtimeDocRef, ref, userId, realtimeUserId, userColor, shouldCreateUser) {
      this.ref_ = ref;
      ...
      this.documentFullText_ = ""
			...
			this.realtimeUserId_ = realtimeUserId;
				if(shouldCreateUser) {
					if (userId && realtimeUserId) {
						self.setUserId(userId, realtimeUserId);
						self.setColor(userColor || colorFromUserId(realtimeUserId));
						...
						// sahil here is when document gets ready
						self.on('ready', function () {
							//self.monitorCursors_();
						});
					} else {
						throw e;
						console.error("No user found to initialize")
					}
		...
		FirebaseAdapter.prototype.dispose = function () {
			...
			if (this.realTimeUserRef_) {
				this.realTimeUserRef_.child('cursor').remove();
				this.realTimeUserRef_.child('color').remove();
			}
      ...
      if (this.realTimeUserRef_) {
        ...
        // (if a future user takes our old name).
        this.realTimeUserRef_.child('cursor').remove();
        this.realTimeUserRef_.child('cursor').onDisconnect().cancel();
        this.realTimeUserRef_.child('color').remove();
        this.realTimeUserRef_.child('color').onDisconnect().cancel();
      }
			this.userId_ = userId;

      this.realTimeUserRef_ = this.realtimeDocRef.child('users').child(userId);

			this.initializeUserData_();
		};

		FirebaseAdapter.prototype.setAuthor = function(userId) {
			return this.ref_.get().then(function(s){
				if(s.exists) {
					return s.ref.update({
						author: userId
					})
		...
		FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
			...
			function doTransaction(revisionId, revisionData) {
				...
				// TODO transaction
				return self.ref_.collection('history').doc(revisionId).set(revisionData).then(function () {
					return revisionData
				})
					.catch(function (error) {
						console.log("Transaction failed: ", error, revisionData);
						throw error;
					});
				/*return firebase.firestore().runTransaction(function (transaction) {
				 // This code may get re-run multiple times if there are conflicts.

				 transaction.update(revisionRef, revisionData);
				 }).then(function (revisionDoc) {

				 console.log("Transaction successfully committed!", revisionData);
				 return revisionDoc;
				 }).catch(function (error) {
				 console.log("Transaction failed: ", error, revisionData);
				 throw error;
				...
				/*self.ref_.collection('history').doc(revisionId).transaction(function(current) {
				 if (current === null) {
				 return revisionData;
				 }
			...
			doTransaction(revisionId, {
				...
				a: self.userId_,
				o: operation.toJSON(),
				t: firebase.firestore.Timestamp.now().toMillis(),
        		c: self.color_
			}).then(function (c) {
		...
		FirebaseAdapter.prototype.sendCursor = function (obj) {
			//ss this is null in first initialize
      this.realTimeUserRef_.child('cursor').set(obj);
			this.cursor_ = obj;
		};

		FirebaseAdapter.prototype.setColor = function (color) {
      this.realTimeUserRef_.child('color').set(color);
      this.color_ = color;
		};
		...
		FirebaseAdapter.prototype.initializeUserData_ = function () {
			// TODO think about this
			this.realTimeUserRef_.child('cursor').onDisconnect(function() {

      }).remove();
			this.realTimeUserRef_.child('color').onDisconnect().remove();

			//ss first initialize null , null
			this.sendCursor(this.cursor_ || null);
			this.setColor(this.color_ || null);
		};

		FirebaseAdapter.prototype.monitorCursors_ = function () {
      var usersRef = this.realtimeDocRef.child('users'), self = this;

      function childChanged(childSnap) {
        // debugger;
        var userId = childSnap.key;
        var userData = childSnap.val();
        //triggering cursor event
    ...
    FirebaseAdapter.prototype.fetchDocument_ = function () {
      var self = this;
      return this.ref_.get().then(function (s) {
        if (s.exists) {
          if (self.zombie_) {
          ...
          if (s.get("checkpoint")) {
            var revisionId = s.get("checkpoint").id, op = s.get("checkpoint").o, author = s.get("checkpoint").a;
          } else {
            var revisionId = null, op = null, author = null;
          }

          if (op != null && revisionId != null && author !== null) {
            self.pendingReceivedRevisions_[revisionId] = {o: op, a: author};
            return self.fetchOperations_(revisionFromId(revisionId) + 1, self.pendingReceivedRevisions_);
          } else {
           return self.fetchOperations_(0)
          }
        } else {
          return ""
        }
    ...
    FirebaseAdapter.prototype.fetchOperations_ = function (revision, pendingRevisionsO) {
      var historyRef = this.ref_.collection('history').orderBy("rId").startAt(revisionToId(revision));
      var pendingRevisions = pendingRevisionsO || [];
      var self = this;
      revision = revision === 0 ? revision : revision - 1;
      var doc = ""
      return new Promise(function(resolve, reject) {
        historyRef.onSnapshot(function (revisionSnapshot) {
          revisionSnapshot.docChanges().forEach(function (change) {
            if (change.type === "added") {
              var revisionId = change.doc.id;
              pendingRevisions[revisionId] = change.doc.data();
            }
          })
          var revisionId = revisionToId(revision), pending = pendingRevisions;
          while (pending[revisionId] != null) {
            var operationH = TextOperation.fromJSON(pending[revisionId]);
            if (!operationH) {
              // If a misbehaved client adds a bad operation, just ignore it.
              utils.log('Invalid operation.', self.ref_.toString(), revisionId, pending[revisionId]);
            } else {
              self.document_ = self.document_.compose(operationH);
            }

            delete pending[revisionId];
            revision++;
            revisionId = revisionToId(revision);
          }
          const data = self.document_.ops.map(
            r => {
              if (!r.attributes.l) {
    ...
    FirebaseAdapter.prototype.monitorHistoryStartingAt_ = function (revision) {
      ...
      setTimeout(function () {
        historyRef.onSnapshot(function (revisionSnapshot) {
          revisionSnapshot.docChanges().forEach(function (change) {
            if (change.type === "added") {
              var revision = change.doc.data();
              var revisionId = change.doc.id;
              self.pendingReceivedRevisions_[revisionId] = change.doc.data();
              if (self.ready_) {
		...
		FirebaseAdapter.prototype.parseRevision_ = function (data) {
			...
			var op = null;
			try {
				op = TextOperation.fromJSON(data);
			}
			catch (e) {
				return null;
			}
			...
			return {author: data.a, operation: op, color: data.c}
		};
		...
		FirebaseAdapter.prototype.saveCheckpoint_ = function () {
			...
			this.ref_.update(checkpointData).catch(function (error) {
				console.error("Error while writing checkpoint: ", error.stack, checkpointData);
			});
		};
	...
	firepad.Client = (function () {
		...
		Client.prototype.applyClient = function (operation) {
			this.setState(this.state.applyClient(this, operation)); // goes to Synchronized.prototype.applyClient
		};
	...
	firepad.EditorClient = (function () {
		...
		function EditorClient(serverAdapter, editorAdapter) {
          ...
          // debugger;
					if (cursor) {
						...
						const cursorJson = Cursor.fromJSON(cursor);
						client.updateCursor(cursorJson);
					} else {
						client.removeCursor();
					}
				}
		...
		EditorClient.prototype.onChange = function (textOperation, inverse) {
			...
			this.applyClient(textOperation); // Client.prototype.applyClient as Editor client inherits client
		};
...
// Line Attributes
	...
	firepad.RichTextCodeMirror = (function () {
		...
		RichTextCodeMirror.prototype.replaceText = function (start, end, text, attributes, origin, color) {
		  this.changeId_++;
			...
			if (color) {
				if (to === null) {
					to = cm.posFromIndex(start);;
					to.ch = to.ch + text.length
				}
				  cm.markText(from, to, {css: "color : " + color});
			}
		};

		RichTextCodeMirror.prototype.insertText = function (index, text, attributes, origin, color) {
			// debugger;
		  var cm = this.codeMirror;
			var cursor = cm.getCursor();
			var resetCursor = origin == 'RTCMADAPTER' && !cm.somethingSelected() && index == cm.indexFromPos(cursor);
			this.replaceText(index, null, text, attributes, origin, color);
		...
		RichTextCodeMirror.prototype.onCodeMirrorChange_ = function (cm, cmChanges) {
			...
			for (var i = 0; i < changes.length; i++) {
				...
				if (removed.length > 0) {
					...
					for (var j = 0; j < oldAnnotationSpans.length; j++) {
						...
						newChanges.push({
							start: start, end: start + span.length, removedAttributes: span.annotation.attributes,
			...
			if (newChanges.length > 0) {
				// goes RichTextCodeMirrorAdapter.prototype.onChange
...
// TODO: Can this derive from CodeMirrorAdapter or similar?
  ...
  firepad.RichTextCodeMirrorAdapter = (function () {
      ...
      // sahil: Editor: 1. type character
			if (changes[0].origin !== 'RTCMADAPTER') {
				var pair = RichTextCodeMirrorAdapter.operationFromCodeMirrorChanges(changes, this.cm);
				this.trigger('change', pair[0], pair[1]); // goes to EditorClient.prototype.onChange  because of E
			}
		};
    ...
    // sahil- here is test entered or deleted
		...
		RichTextCodeMirrorAdapter.prototype.applyOperation = function (operation) {
			...
			for (var i = 0, l = ops.length; i < l; i++) {
				...
				if (op.isRetain()) {
					...
					this.rtcm.insertText(index, op.text, op.attributes, 'RTCMADAPTER', op.color);
	...
	firepad.FirepadPure = (function (global) {

		var TextOperation = firepad.TextOperation;
		var FirebaseAdapter = firepad.FirebaseAdapter;

		function FirepadPure(realtimeDb, ref, user) {
			//ss 3. Here is is called pure
			if (!(this instanceof FirepadPure)) {
				return new FirepadPure(realtimeDb, ref, user || {});
			}

      this.realtimeDbRef_ = realtimeDb.ref(ref.id.toString())
			this.fileId_ = ref.id;

			this.textOperation_ = firepad.TextOperation;

			this.userId_ = user.userId;

			this.userColour_ = user.colour || colorFromUserId(this.userId_);
			this.firebaseAdapter_ = new FirebaseAdapter(this.realtimeDbRef_, ref, this.userId_, null, this.userColour_);


			var defaultOperation = this.generateInitialOperation();

			this.firebaseAdapter_.on('ready', this.createEmptyDocument(defaultOperation));

		}

		firepad.utils.makeEventEmitter(FirepadPure, ['ready']);

		FirepadPure.prototype.generateInitialOperation = function() {
			var defaultText = "";
			var operationDefault = new this.textOperation_().retain(defaultText.length);
			operationDefault = new this.textOperation_()
				.retain(0)
				['delete'](0)
				.insert(defaultText, null)
				.retain(0)
				.compose(operationDefault);
			return  operationDefault;
		};

		FirepadPure.prototype.createEmptyDocument = function(operationDefault){

			var self = this;
			self.firebaseAdapter_.setAuthor(self.userId_).then(function() {
				self.ready_ = true;
				if (self.firebaseAdapter_.isHistoryEmpty()) {
					self.firebaseAdapter_.sendOperation(operationDefault, function (result) {

						self.trigger('ready', self.fileId_);
					})
					//this.client_.sendOperation
				} else {

					self.trigger('ready', self.fileId_);
				}
			}).catch(function (e) {
				console.error("Error while initializinf firebase adapter", e.stack)
				throw e;
			});
		};
  ...
  firepad.FirepadDocument = (function (global) {
    var FirebaseAdapter = firepad.FirebaseAdapter;
    function FirepadDocument(documentId, userId, db, realtimeDoc) {

      var docRef = db.collection("realTimeDocuments").doc(documentId);

      return docRef.get().then(function(doc) {
        if (doc.exists) {
          this.firebaseAdapter_ = new FirebaseAdapter(realtimeDoc, docRef, userId, userId, null);

          return this.firebaseAdapter_.fetchDocument_()
        } else {
	...
	firepad.Firepad = (function (global) {
		...
		//ss 2. Initialises FirepadPure
    var FirepadDocument = firepad.FirepadDocument;
		var FirepadPure = firepad.FirepadPure;
		...
		function Firepad(realtimeDbRef, ref, place, options) {
			//sahil - entry point

			if (!(this instanceof Firepad)) {
				return new Firepad(realtimeDbRef, ref, place, options);
			}
      ...
      this.realtimeDoc_ = realtimeDbRef;
			this.zombie_ = false;
			...
			// Creates a div element with class firepad and put contents for toolbar
			...
			this.options_ = options || {};
			// sahil - false here is default value
			...
			if (this.getOption('richTextToolbar', false)) {
				// sahil remove toolbar for now
			  //this.addToolbar_();
				this.firepadWrapper_.className += ' firepad-richtext';
			}
			// sahil - no logo for now
			...
			var realtimeUserId = options.realtimeUserId;
			var userColor = this.getOption('userColor', colorFromUserId(userId));

			// sahil - understood till here :-)
			this.entityManager_ = new EntityManager();

			this.firebaseAdapter_ = new FirebaseAdapter(this.realtimeDoc_, ref, userId, realtimeUserId, userColor, true);
			if (this.codeMirror_) {
			...
			var defaultText = "";
			var operationDefault = new TextOperation().retain(defaultText.length);
			operationDefault = new TextOperation()
				.retain(0)
				['delete'](0)
				.insert(defaultText, null)
				.retain(0)
			...
			this.firebaseAdapter_.on('ready', function () {
					if(self.options_.readOnly) {
            self.codeMirror_.setOption("readOnly", true);
            self.codeMirror_.setOption("nocursor", true);
					} else {
            self.codeMirror_.setOption("readOnly", false);
					}


					self.ready_ = true;

					var defaultText = self.getOption('defaultText', "");
    ...
    Firepad.firepadDocument = FirepadDocument;
		//ss 1. Initializes here
		Firepad.backEnd = FirepadPure;
		...
		Firepad.prototype.hasWritePermission = function(ref, userId) {
			return ref.collection("permissions").doc(userId).get().then(function(snap) {

				if(snap.exists) {
					return snap.data().write
				} else {
					return true;
				}
			})
