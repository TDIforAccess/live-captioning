var FirepadUserList = (function() {
	function FirepadUserList(ref, place, placeHeading, userId, realtimeUserId, displayName) {
		if (!(this instanceof FirepadUserList)) {
			return new FirepadUserList(ref, place, placeHeading, userId, realtimeUserId, displayName);
		}
		this.ref_ = ref;
		this.userId_ = userId;
		...
		this.heading_ = this.makeHeading_();
		place.appendChild(this.userList_);
		placeHeading.appendChild(this.heading_);
	}
    ...
    this.ref_.once("value").then(usersSnapshot => {
        const data = usersSnapshot.val();
        const usersSize = Object.keys(data).length;
        setTextContent(counterSpan, "" + usersSize);
      })
    ...
    this.firebaseOn_(myUserRef.child('color'), 'value', function(colorSnapshot) {
      ...
      if (isValidColor(color)) {
		...
		var divName = elt('div', this.displayName_, { type: 'text', 'class': 'firepad-userlist-name'} );
